...
class EventEdit extends Component {
    ...
    handleGetEditProps = () => {
        ...
        let imageByUrl = false;
        if(typeof(data) !== "undefined"){
            imageData.image_source = `${Setting.isOnline ? Setting.onlinePath : Setting.offlinePath}${data.event.event_poster}`
            if(data.event.event_poster.split('/')[0] !== "assets"){
                imageByUrl = true;
                imageData.image_source = `${data.event.event_poster}`
            }
            this.setState({
                event : data.event,
            ...
            .then((result) => {
                if(result.status){
                    let event_data = result.data[0];
                    imageData.image_source = `${Setting.isOnline ? Setting.onlinePath : Setting.offlinePath}${event_data.event_poster}`
                    if(event_data.event_poster.split('/')[0] !== "assets"){
                        imageByUrl = true;
                        imageData.image_source = `${event_data.event_poster}`
                    }
                    this.setState({
                        event : event_data,
                        imageByUrl : imageByUrl,
                            ...
                            <div className="add-form row justify-content-center">
                                <div className="col-12 col-sm-12 col-md-8 col-lg-8">
                                    ...
                                    <div className="form-group row">
                                        ...
                                        <div className="col-12 col-sm-12 col-md-9 col-lg-9">
                                            <button onClick={this.editEvent} className="btn btn-primary">Simpan</button>
...
export default ContentWrapper(ContextConsumer(EventEdit));